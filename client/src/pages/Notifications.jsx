...
import { X, Trash2, CheckCheck, Trash, Ship, Bell } from "lucide-react";
      ...
      <div className="max-w-4xl mx-auto space-y-6">
          ...
          <div className="flex gap-2">
            {unreadCount > 0 && (
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 rounded-lg bg-teal-600 text-white hover:bg-teal-700 transition font-medium"
        ...
        {notifications.length === 0 ? (
          <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-12 text-center">
            ...
            <p className="text-slate-900 text-lg font-semibold">No notifications yet</p>
          ...
          <div className="space-y-3">
            {notifications.map((notification) => (
              <div
                ...
                className={`p-5 rounded-2xl border cursor-pointer transition-all duration-200 ${notification.is_read
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-3 mb-2">
                      {!notification.is_read && (
                        <div className="h-2.5 w-2.5 bg-blue-600 rounded-full shrink-0 shadow-sm shadow-blue-400/50" />
                      )}
                      <h3 className={`font-bold text-lg ${notification.is_read ? 'text-slate-700' : 'text-slate-900'}`}>
                    ...
                    <div className="flex items-center gap-4 text-sm text-slate-500">
                      <span className="font-semibold text-slate-700 flex items-center gap-1.5">
                        <Ship size={14} className="text-blue-500" /> {notification.vessel_name}
                      </span>
                      <span className="text-xs text-slate-400">
                        {notification.vessel_type} • {notification.vessel_flag}
                      </span>
                      <span className="text-xs ml-auto font-medium text-slate-400">
                  ...
                  <div className="flex gap-2 shrink-0">
                    {!notification.is_read && (
                      <button
                        ...
                        className="p-2 rounded-lg hover:bg-white hover:shadow-sm border border-transparent hover:border-slate-200 transition text-slate-400 hover:text-green-600"
                        title="Mark as read"
                      >
                        <CheckCheck className="h-4 w-4" />
                      </button>
                    ...
                    <button
                      ...
                      className="p-2 rounded-lg hover:bg-white hover:shadow-sm border border-transparent hover:border-slate-200 transition text-slate-400 hover:text-red-600"
      ...
      {showModal && selectedNotification && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl shadow-xl max-w-lg w-full max-h-[90vh] overflow-y-auto">
            {/* Modal Header */}
            <div className="sticky top-0 flex items-center justify-between p-6 border-b border-sky-100 bg-gradient-to-r from-blue-50 to-cyan-50">
              <div>
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="flex items-center gap-2">
                <span className="text-sm font-semibold text-slate-500">Event Type:</span>
                <span className="bg-slate-100 text-slate-700 px-3 py-1 rounded-full font-medium border border-slate-200 text-xs uppercase tracking-wider">
              ...
              {/* Message */}
              <div>
                <h3 className="text-sm font-semibold text-slate-900 mb-2">Message Content</h3>
                <p className="text-slate-700 text-base leading-relaxed bg-slate-50 p-4 rounded-xl border border-slate-100">
              ...
              <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm">
                ...
                <div className="grid grid-cols-2 gap-y-4 gap-x-8 text-sm">
                  <div>
                    <p className="text-slate-400 text-xs uppercase tracking-wider font-semibold mb-1">IMO Number</p>
                    <p className="font-mono font-medium text-slate-700 bg-slate-50 inline-block px-1.5 rounded">
                  ...
                  <div>
                    <p className="text-slate-400 text-xs uppercase tracking-wider font-semibold mb-1">Flag</p>
                    <p className="font-medium text-slate-900">
                  ...
                  <div>
                    ...
                    <p className={`font-bold ${selectedNotification.is_read ? 'text-slate-400' : 'text-blue-600'}`}>
              ...
              <div className="flex items-center justify-between text-xs text-slate-400 pt-2 border-t border-slate-100">
                <p>Notification ID: #{selectedNotification.id}</p>
            ...
            <div className="sticky bottom-0 flex gap-3 p-6 border-t border-slate-200 bg-slate-50">
              {!selectedNotification.is_read && (
                <button
                  ...
                  className="flex-1 flex items-center justify-center gap-2 px-4 py-2 rounded-lg bg-teal-600 text-white hover:bg-teal-700 transition font-medium"
                >
                  <CheckCheck className="h-4 w-4" />
                  Mark as Read
                </button>
